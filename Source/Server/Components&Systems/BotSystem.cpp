...
void BotSystem::dodge_bullets(EntityID ennemy)
{
    ...
    for (EntityID ent : EntityViewer<Position>(*_em.get())) {
        ...
        std::chrono::duration<float> diff = std::chrono::system_clock::now() - _em->Get<AI>(ennemy)->lastTimeMoved;

        if (diff.count() > _em->Get<AI>(ennemy)->moveCooldown) {
            _em->Get<AI>(ennemy)->lastTimeMoved = std::chrono::system_clock::now();
            _em->Get<AI>(ennemy)->isMoving = true;
            _em->Get<HasUpdated>(ennemy)->updated = true;
...
void BotSystem::fire_weapon_ennemy(EntityID ennemy)
{
    ...
    std::chrono::duration<float> diff = std::chrono::system_clock::now() - _em->Get<AI>(ennemy)->lastTimeShot;
    
    if (diff.count() > _em->Get<AI>(ennemy)->shootCooldown) {
        _em->Get<Input>(ennemy)->_inputQueue.emplace_back(SHOOT);
        _em->Get<AI>(ennemy)->lastTimeShot = std::chrono::system_clock::now();
    }
}
