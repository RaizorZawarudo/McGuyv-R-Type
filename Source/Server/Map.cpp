...
void Map::fillMapData(std::string MapPath, std::vector<RL::Drawable3D*> zonesModels)
{
    ...
    std::vector<std::string> loopZoneWavesParsed;
    this->_maxStage = parsedCsv.size() - 2;
    ...
    for (long unsigned int i = 1; i < parsedCsv.size(); i++) {
        ...
        for (long unsigned int j = 0; j < parsedCsv[i].size(); j++) {
            ...
            if (j == 1) { //here add the waves of the small path
                loopZoneWavesParsed = splitStr(parsedCsv[i][j], ";");
                this->_repeatPathWaves.push_back(loopZoneWavesParsed);
            }

            if (j == 2) {// wave amounts
                this->_waveamounts.emplace_back(stoi(parsedCsv[i][j]));
            }
            ...
            if (j == 4) { //here add the boss of the bossroom
                this->_bossRoomBosses.push_back(parsedCsv[i][j]);
            }
    ...
    std::cout << "MAP NAME = " << this->_mapName << std::endl;
        for ( int i = 0 ; i < _repeatPathsnames.size(); i++) {
            ...
            for (int a = 0; a < _repeatPathWaves[i].size(); a++) {
                std::cout << "\t\t"<< _repeatPathWaves[i][a] << " is a wave in " << _repeatPathsnames[i] << std::endl;
...
void Map::updateMapQueue()
{
    for (int i = 0; i < this->_mapQueue.size(); i++) {
        ...
        if (this->_isFightingBoss == true && !this->_mapQueue[0]._isBossRoom )
...
void Map::refillMapQueue()
{
    ...
    if (this->_isFightingBoss == false) { // this is the logic to add simple paths when not fighting a boss, so the infinite straight loop without anything else
        ...
        if (queueSize == 0) {
            ...
            newMapSection._length = this->_repeatPaths.at(this->_currentStage)._length;
            this->_mapQueue.emplace_back(newMapSection);
            std::cout << "Position is " << newMapSection._zPosition<< " length " << newMapSection._length << std::endl;
            return;
        }
    }
    if (this->_isFightingBoss == true) {
        if (queueSize != 6 && this->_hasSpawnedBossroom == false) {
            ...
            std::cout << " CREATING BOSS ROMM NUMBER : " << _currentStage << std::endl;
            std::cout << "Position is " << newMapSection._zPosition<< " length " << newMapSection._length << std::endl;
        ...
        if (queueSize < 6 && this->_hasSpawnedBossroom == true) {
            ...
            //debugging
            std::cout << _isFightingBoss << std::endl;
...
std::vector<std::vector<std::string>> Map::getRepeathPathWaves()
{
    return this->_repeatPathWaves;
}

std::vector<int> Map::getWaveAmountsVector()
{
    return this->_waveamounts;
}

std::vector<std::string> Map::getBossRoomBossName()
{
    return this->_bossRoomBosses;
}
...
float Map::getCurrentSpeed()
{
    return this->_currentSpeed;
}

bool Map::getHasSpawnedBoss()
{
    return this->_hasSpawnedBossMob;
}

bool Map::getHasSpawnedBossRoom()
{
    return this->_hasSpawnedBossroom;
}
...
void Map::setCurrentWavePlusOne()
{
    this->_currentWave += 1;
}

void Map::setFightingBossFalse()
{
    this->_isFightingBoss = false;
}

void Map::setFightingBossTrue()
{
    this->_isFightingBoss = true;
}

void Map::setHasSpawnedBossFalse()
{
    this->_hasSpawnedBossroom = false;
}

void Map::setMapSpeed(float gas)
{
    this->_scrollSpeed += gas;
}

void Map::bossIsDown()
{
    
    this->setHasSpawnedBossFalse();
    this->setSpawnedBoss();
    this->setFightingBossFalse();
    if (this->_currentStage < this->_maxStage) {
...
void Map::setSpawnedBoss()
{
    this->_hasSpawnedBossMob = !this->_hasSpawnedBossMob;
}