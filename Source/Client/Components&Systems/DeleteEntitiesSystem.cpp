...
void DeleteEntitiesSystem::update(std::vector<EntityID> &allEntities)
{
    for (EntityID ent : EntityViewer<Position, IsAlive>(*_em.get())) {
        ...
        if (_em->Get<EntityModelType>(ent)->modelType == RL::ModelType::POWERUP) {
            std::chrono::duration<float> diff = std::chrono::system_clock::now() - _em->Get<Timer>(ent)->lastTimeClocked; // SEGV HERE
            if (diff.count() > _em->Get<Timer>(ent)->cooldowninseconds) {
                _em->Remove<Timer>(ent);
                _em->Remove<Position>(ent);
                _em->Remove<EntityModelType>(ent);
                _em->Remove<Input>(ent);
