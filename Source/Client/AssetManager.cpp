...
{
    std::string zonesModelCSVPath = "Source/Client/Assets/Models/Zones/Zones.csv";
    std::string spacecraftModelCSVPath = "Source/Client/Assets/Models/Spacecrafts/Spacecrafts.csv";
    std::string ennemyModelCSVPath = "Source/Client/Assets/Models/Ennemies/Ennemies.csv";
    std::string projectileCSVPath = "Source/Client/Assets/Models/Projectiles/Projectiles.csv";
    std::string mapBackgroundCSVPath = "Source/Client/Assets/BackgroundTextures/Backgrounds.csv";
    std::string obstacleCSVPath = "Source/Client/Assets/Models/Obstacles/Obstacles.csv";
    std::string explosionCSVPath = "Source/Client/Assets/Models/Explosions/Explosions.csv";
    std::string effectsCSVPath = "Source/Client/Assets/Models/Effects/Effects.csv";
    std::string powerUpCSVPath = "Source/Client/Assets/Models/PowerUps/PowerUps.csv";
    std::string iconsUICSVPath = "Source/Client/Assets/Icons/Icons.csv";
    std::string fontsCSVPath = "Source/Client/Assets/Fonts/Fonts.csv";
    std::string obstacleWavesCSVPath = "Source/Client/Assets/ObstacleWaves/obstacleWaves.csv";
    std::string ennemyWavesCSVPath = "Source/Client/Assets/EnnemyWaves/ennemyWaves.csv";
    std::string scenarioCSVPath = "Source/Client/Assets/Scenarios/Scenarios.csv";
    ...
    //load all Scenarios ( scenarios is what determins the order of the waves and bosses in a map)
    this->_scenarios = loadAllScenarios(scenarioCSVPath);

    for (int i = 0; i < _scenarios.size(); i++)
        std::cout << "SCENARIO NAME : " << _scenarios[i].scenarioName << std::endl;
}
...
{
    std::string allMapsCSVPath = "Source/Client/Maps/maps.csv";
...
std::vector<Font> AssetManager::loadAllFonts(const std::string &fontCSVPath)
{
    ...
    for (long unsigned int i = 1; i < parsedCsv.size(); i++) {
        ...
        newfont = LoadFontEx(fontpath.c_str(), 25, 0, 250); // SIGIIII
...
std::vector<Wave_t> AssetManager::loadEnnemyWaves(const std::string &ennemyWavesCSVPath)
{
    std::vector<std::vector<std::string>> parsedCsv = csvToTable(ennemyWavesCSVPath);
    Wave_t newWave;
    std::string waveData;
    std::vector<Wave_t> ennemyWaves;
...
std::vector<Scenario_t> AssetManager::loadAllScenarios(const std::string &scenarioCSVPath)
{
    std::vector<std::vector<std::string>> parsedCsv = csvToTable(scenarioCSVPath);
    Scenario_t newScenario;
    ...
    for (long unsigned int i = 1; i < parsedCsv.size(); i++) {
        ...
        for (long unsigned int j = 0; j < parsedCsv[i].size(); j++) {
            if (j == 0)
                newScenario.scenarioName = parsedCsv[i][j];
            if (j == 1) {
                newScenario.waveNames = loadScenarioData(parsedCsv[i][j]);
                newScenario.waveTypes = loadScenarioDataAgain(parsedCsv[i][j]);
            }
        }
 ...
 std::vector<std::string> AssetManager::loadScenarioData(const std::string &individualScenarioPath)
 {
    std::vector<std::vector<std::string>> parsedCsv = csvToTable(individualScenarioPath);
    std::vector<std::string> scenarioWaveNames;
    std::string waveName;
    ...
    for (long unsigned int i = 1; i < parsedCsv.size(); i++) {
        ...
        for (long unsigned int j = 0; j < parsedCsv[i].size(); j++) {
            if (j == 0)
                waveName = parsedCsv[i][j];
        }
        scenarioWaveNames.push_back(waveName);
        
    }
    return scenarioWaveNames; 

 }

 std::vector<std::string> AssetManager::loadScenarioDataAgain(const std::string &individualScenarioPath)
 {
    std::vector<std::vector<std::string>> parsedCsv = csvToTable(individualScenarioPath);
    std::vector<std::string> scenarioWaveTypes;
    std::string waveType;
    ...
    for (long unsigned int i = 1; i < parsedCsv.size(); i++) {
        ...
        for (long unsigned int j = 0; j < parsedCsv[i].size(); j++) {
            if (j == 1)
                waveType = parsedCsv[i][j];
        }
        scenarioWaveTypes.push_back(waveType);        
    }
    return scenarioWaveTypes;
 }
...
std::vector<RL::Drawable3D*> AssetManager::getExplosionModels()
{
    return this->_explosionModelsAnim;
}
...
std::vector<Scenario_t> AssetManager::getScenarios()
{
...
std::chrono::system_clock::time_point AssetManager::getStartTimestamp()
{
...
void AssetManager::setCurrentClientID(unsigned long long id)
{
...
void AssetManager::startTime()
{
    this->_startTimestamp = std::chrono::system_clock::now();
}

bool AssetManager::getRacerMode()
{
    return this->_racerMode;
}

void AssetManager::setRacerMode()
{
    this->_racerMode = !this->_racerMode;
}