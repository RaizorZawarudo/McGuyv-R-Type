...
void CollisionSystem::bullet_collisions(EntityID projectile, EntityID other)
{
    ...
    if (CheckCollisionBoxes(projectileCollider->collider, otherCollider->collider)) {
        ...
        if (_em->Get<Hp>(other)->hp <= 0) {
            ...
            //here we integrate if the ennemy is a boss, we move to the next stage of the map / scenario
            if (_assetManager->getSpecificDrawableWithType(_em->Get<ModelName>(other)->modelname, RL::ModelType::ENNEMY)->getStyle() == "boss") {
                _assetManager->getMaps().at(_assetManager->getCurrentMapBeingPlayed())->bossIsDown();
...
void CollisionSystem::body_collisions(EntityID ennemy, EntityID other)
{
    //if its two spaceships return
    if (_em->Get<EntityModelType>(other)->modelType == RL::ModelType::SPACESHIP && _em->Get<EntityModelType>(ennemy)->modelType == RL::ModelType::SPACESHIP)
        return;
    //if its not a spaceship a the first entity return;
    if (_em->Get<EntityModelType>(ennemy)->modelType != RL::ModelType::ENNEMY)
        return;
    //if its not a spaceship a the first entity return;
    if (_em->Get<EntityModelType>(other)->modelType != RL::ModelType::ENNEMY && _em->Get<EntityModelType>(other)->modelType != RL::ModelType::SPACESHIP)
        return;
    //if im myself return
    if (ennemy == other)
        return;

    Collider* ennemyCollider = _em->Get<Collider>(ennemy);
    Collider* otherCollider = _em->Get<Collider>(other);
    if (CheckCollisionBoxes(ennemyCollider->collider, otherCollider->collider)) {       

        //reduce other shield or hp by ennemy hp
        if (_em->Get<Shield>(other)) {
            _em->Get<Shield>(other)->shieldActive ? _em->Get<Shield>(other)->shield -= _em->Get<Hp>(ennemy)->hp / 2 : _em->Get<Hp>(other)->hp -= _em->Get<Hp>(ennemy)->hp;
            if ( _em->Get<Shield>(other)->shield < 0) {
                _em->Get<Hp>(other)->hp += _em->Get<Shield>(other)->shield;
                _em->Get<Shield>(other)->shield = 0;
            }
            ...
            _em->Get<Hp>(other)->hp -= _em->Get<Hp>(ennemy)->hp;
        }
        //check if other hp is below zero : if yes set it to dead and create an explosion entity with the explosion path stored in the other´s components ==>  createExplosion(_em->Get<ExplosionName>(other)->name);
        if (_em->Get<Hp>(other)->hp <= 0) {
            _em->Get<IsAlive>(other)->alive = false;
            Position* ennemyPos = _em->Get<Position>(ennemy);
            ModelName* otherName = _em->Get<ModelName>(other);
            create_explosion(ennemyPos->pos, otherName->explosionname);
         }
        _em->Get<IsAlive>(ennemy)->alive = false;
        Position* ennemyPos = _em->Get<Position>(ennemy);
        ModelName* ennemyName = _em->Get<ModelName>(ennemy);
        create_explosion(ennemyPos->pos, ennemyName->explosionname);
    }
}
